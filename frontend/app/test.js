...
export default function Cart() {
    ...
    useEffect(() => {
        const fetchCartDetails = async () => {
            ...
            try {
                ...
                const cartResponse = await fetch(`http://localhost:8000/api/show_cart/show_detail_cart/`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    const handleQuantityChange = async (product_id, action) => {
        try {
            ...
            const apiEndpoint = action === 'add' ? 'plus' : 'minus';
            const response = await fetch(`http://localhost:8000/api/cart/${apiEndpoint}/`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            setCartDetails(prevDetails => 
                prevDetails.map(cart => {
                    const updatedProducts = cart.cart_products
                        .map(item => {
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <dl className="flex items-center justify-between gap-4">
                                            ...
                                            {/* sum price */}
                                            {cartDetails.map((cart, index) => (
                                                <dd key={index} className="text-base font-medium text-gray-900">{cart.totalPrice}</dd>
                                            ))}
                                        </dl>
